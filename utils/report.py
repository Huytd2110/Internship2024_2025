...
# Khai báo ENV theo từng ứng dụng
APP_ENVS = {
    "DVWA": {
    ...
    "JuiceShop": {
        ...
        "Tools Used": "Python requests, BeautifulSoup4, Selenium",
    }
}

def get_env_info(extra_info=None):
...
def generate_full_report(scan_results, target_name="Multi App Pentest"):
...
def save_report_markdown(report, filename="output/full_report.md"):
    with open(filename, "w", encoding="utf-8") as f:
        ...
        # --- GROUP THE VULNS BY target_app ---
        app_vuln_groups = defaultdict(list)
        for v in report["vulnerabilities"]:
            app = v.get("target_app", "DVWA")
            app_vuln_groups[app].append(v)

        for app, vulns in app_vuln_groups.items():
            f.write(f"## Test Environment: {app}\n\n")
            env = get_env_info(APP_ENVS.get(app, {}))
            ...
            for step in METHODOLOGY:
                f.write(f"- {step}\n")
            f.write("\n---\n")
            # Group test cases by type inside each app
            module_groups = defaultdict(list)
            for v in vulns:
                module_type = v.get("type", "Other")
                module_groups[module_type].append(v)
            for module_type, vulns_by_type in module_groups.items():
                f.write(f"\n### {module_type} Test Cases\n\n")
                f.write("| # | Payload | Status | Severity |\n")
                f.write("|---|---------|--------|----------|\n")
                for i, v in enumerate(vulns_by_type):
                    f.write(
...
def save_report_pdf(report, filename="output/full_report.pdf"):
    ...
    for app, vulns in app_vuln_groups.items():
        # Test Environment for each app
        env = get_env_info(APP_ENVS.get(app, {}))
        pdf.set_font("Arial", 'B', 13)
        pdf.cell(0, 10, f"Test Environment: {app}", 0, 1)
    ...
    # Executive summary cho toàn bộ report (không tách theo app)
